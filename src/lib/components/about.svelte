<script lang="ts">
  export let className: string = "";
</script>

<div class={className}>
  <p>
    <span class="bg-[#4750DD] text-[#EAFFD9]">Martín Stier</span> is a
    fourth-year student at the
    <span class="bg-[#F5BF0F] text-[#000000]">University of Michigan </span>
    College of Engineering majoring in Computer Science.
    <!-- . He is majoring in Computer Science, with minors in Complex Systems and
    International Studies. He is also a member of the
    <span class="bg-[#F3000E] text-[#EAFFD9]">Society of Les Voyageurs</span>. -->
  </p>
</div>
